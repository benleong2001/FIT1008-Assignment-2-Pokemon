...
class CircularQueue(Queue[T]):
    ...
    def __str__(self):
        """ Magic method constructing a string representation of the CircularQueue.
        ...
        if self.rear <= self.front:
            return ", ".join(str(x) if isinstance(x, PokemonBase) and x.is_alive() else str(x) for x in
                             self.array[self.front:len(self.array)] + self.array[0:self.rear])
        else:
            return ", ".join(str(x) if isinstance(x, PokemonBase) and x.is_alive() else str(x) for x in
                             self.array[self.front:self.rear])
